...
export function buildEmailVersion(result: TechnicalSeoScanResult): string {
  ...
  if (report.performance) lines.push("", "PAGE SPEED EVIDENCE", `- Status: ${report.performance.status.replace(/_/g, " ")}`, `- Source: ${value(report.performance.source?.replace(/_/g, " "))}`, `- Mobile score: ${value(report.performance.mobile?.score)}`, `- Mobile LCP: ${report.performance.mobile?.lcpMs ? `${Math.round(report.performance.mobile.lcpMs)} ms` : "Not available"}`, `- Desktop score: ${value(report.performance.desktop?.score)}`);
  if (report.local) lines.push("", "LOCAL SEARCH EVIDENCE", `- Overall status: ${report.local.status.replace(/_/g, " ")}`, `- Profile status: ${(report.local.profileStatus ?? report.local.status).replace(/_/g, " ")}`, `- Profile match method: ${value(report.local.profileMatchMethod?.replace(/_/g, " "))}`, `- Rankings status: ${(report.local.rankingsStatus ?? report.local.status).replace(/_/g, " ")}`, `- Representative query: ${value(report.local.query)}`, `- Organic rank: ${value(report.local.organicRank)}`, `- Map rank: ${value(report.local.mapRank)}`, `- Matched profile: ${value(report.local.profile?.title)}`);
