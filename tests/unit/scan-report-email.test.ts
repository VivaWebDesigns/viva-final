...
describe("scan report email template", () => {
  ...
  it("omits a greeting from both English outreach messages", async () => {
    ...
    expect(source).toContain('search for “{{search_phrase}}.”');
    expect(source).not.toContain('search for “{{search_phrase}}”.');
    expect(source).not.toContain('use searches like “{{search_phrase}}” to find a company.');
