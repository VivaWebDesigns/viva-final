...
export function createSabMcpServer(
  ...
  server.registerTool(
    ...
    sabTool({
      description:
        "Execute exactly one run-state-authorized Local Falcon scan owned by the single Codex orchestrator through the Viva connector's guarded path. Requires an exact approved batch in run_id, enforces testing pauses and credit limits, and creates a durable idempotency reservation before any paid call, optionally saves the exact Place ID, submits exactly once with no automatic retry, verifies the complete echoed scan envelope, and immediately records the report key. A lost or mismatched response becomes an ambiguous durable stop that must be reconciled. An explicitly Matt-approved pre-provider recovery may resume only the same exact preparing_location claim without reserving credits again; all other repeated calls remain read-only receipts.",
