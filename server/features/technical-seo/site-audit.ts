...
export async function buildSiteAudit(args: { rootUrl: string; homepage: TechnicalSeoPageAudit; sitemapUrls: string[]; context: TechnicalSeoAuditContext; issues: TechnicalSeoIssue[]; signal?: AbortSignal }): Promise<TechnicalSeoSiteAudit> {
  ...
  const perfScore = performance.mobile?.score ?? null;
  const gbpScore = local.status === "measured" ? Math.max(0, 100 - (!local.profile ? 65 : 0) - (local.profile && !local.profile.phone ? 15 : 0) - (local.profile && !local.profile.address ? 10 : 0) - (local.profile && !local.profile.category ? 10 : 0)) : null;
  ...
  const localScore = args.context.city && args.context.trade ? Math.min(100, 35 + localMatches * 8 + (local.organicRank && local.organicRank <= 10 ? 20 : 0) + (local.mapRank && local.mapRank <= 3 ? 20 : 0)) : null;
  ...
  const grades: TechnicalSeoSiteAudit["grades"] = [
    ...
    { key: "performance", label: "Page speed", grade: perfScore === null ? "Not assessed" : grade(perfScore), score: perfScore, rationale: performance.status === "measured" ? "Based on the measured mobile Lighthouse performance score." : performance.reason ?? "PageSpeed was not available." },
    { key: "business_profile", label: "Google Business Profile signals", grade: gbpScore === null ? "Not assessed" : grade(gbpScore), score: gbpScore, rationale: local.status === "measured" ? "Based on the matched public business profile fields." : local.reason ?? "Business profile data was not available." },
    { key: "local_seo", label: "Local SEO strength", grade: localScore === null ? "Not assessed" : grade(localScore), score: localScore, rationale: `${localMatches} crawled pages showed contextual trade or location targeting; rankings are included when measured.` },
  ...
  return {
    ...
    coverage: { assessed: ["Technical HTML and crawlability", `Up to ${SCAN_LIMITS.maxCrawlPages} same-origin pages`, "Homepage rendered output", ...(performance.status === "measured" ? ["Google PageSpeed lab data"] : []), ...(local.status === "measured" ? ["Google business and local search data via DataForSEO"] : [])], notAssessed: ["Google Search Console account data", "Google Analytics conversion data", "Backlink quality", "Review-response behavior without a matched profile", "Reliable AI-authorship detection"] },
...
export function buildSiteIssues(audit: TechnicalSeoSiteAudit): TechnicalSeoIssue[] {
  ...
  if (audit.performance.mobile?.score !== null && audit.performance.mobile?.score !== undefined && audit.performance.mobile.score < 50) add("mobile-performance-poor", "Poor measured mobile performance", "high", "Page speed", `Google PageSpeed measured mobile performance at ${audit.performance.mobile.score}/100.`, `LCP ${Math.round(audit.performance.mobile.lcpMs ?? 0)} ms; TBT ${Math.round(audit.performance.mobile.tbtMs ?? 0)} ms; CLS ${audit.performance.mobile.cls ?? "unavailable"}.`, "Slow mobile delivery creates a weaker user experience and can undermine search performance in competitive results.");
  if (audit.performance.mobile?.lcpMs && audit.performance.mobile.lcpMs > 2500) add("mobile-lcp", "Slow mobile Largest Contentful Paint", audit.performance.mobile.lcpMs > 4000 ? "high" : "medium", "Core Web Vitals", `Measured mobile LCP is ${Math.round(audit.performance.mobile.lcpMs)} ms.`, "Google Lighthouse lab measurement.", "The main visible content takes longer than the recommended threshold to appear.");
  if (audit.local.status === "measured" && !audit.local.profile) add("gbp-not-matched", "Google Business Profile not matched", "high", "Google Business Profile", "The provider did not return a confident business profile match for the supplied business name and location.", `${audit.context.businessName}; ${audit.context.city}, ${audit.context.state}`, "The audit cannot confirm a consistent business entity in the local search data returned for the supplied identity.");
  if (audit.local.status === "measured" && audit.local.organicRank === null) add("organic-not-top-ten", "Website absent from the first ten organic results", "high", "Local rankings", `The domain was not present in the first ten organic results for “${audit.local.query}”.`, `Query: ${audit.local.query}; mobile; ${audit.context.city}, ${audit.context.state}.`, "Competitors occupy the highest-visibility organic positions for the representative commercial search.");
  if (audit.local.status === "measured" && audit.local.mapRank === null) add("maps-not-top-ten", "Business absent from the first ten map results", "high", "Local rankings", `The business was not matched in the first ten Google Maps results for “${audit.local.query}”.`, `Query: ${audit.local.query}; mobile; ${audit.context.city}, ${audit.context.state}.`, "The business lacks top-page map visibility for the representative local commercial search.");
  return issues;
}
