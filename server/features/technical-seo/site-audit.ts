...
export async function buildSiteAudit(args: { rootUrl: string; homepage: TechnicalSeoPageAudit; sitemapUrls: string[]; context: TechnicalSeoAuditContext; issues: TechnicalSeoIssue[]; signal?: AbortSignal }): Promise<TechnicalSeoSiteAudit> {
  ...
  const grades: TechnicalSeoSiteAudit["grades"] = [
    ...
    { key: "business_profile", label: "Google Business Profile signals", grade: gbpScore === null ? "Not assessed" : grade(gbpScore), score: gbpScore, rationale: profileStatus === "measured" ? `Based on public profile fields matched by ${local.profileMatchMethod === "google_business_url" ? "the supplied Google Business Profile URL" : "business name and location"}.` : local.profileReason ?? "Business profile data was not available." },
  ...
  return {
    ...
    coverage: { assessed: ["Technical HTML and crawlability", `Up to ${SCAN_LIMITS.maxCrawlPages} same-origin pages`, "Homepage rendered output", ...(performance.status === "measured" ? ["Google PageSpeed Lighthouse lab data"] : performance.status === "estimated" ? ["Controlled local Chromium performance estimate"] : []), ...(profileStatus === "measured" ? ["Google Business Profile public data via DataForSEO"] : []), ...((local.rankingsStatus ?? local.status) === "measured" ? ["Google organic and Maps results via DataForSEO"] : [])], notAssessed: ["Google Search Console account data", "Google Analytics conversion data", "Backlink quality", "Review-response behavior without a matched profile", "Reliable AI-authorship detection", ...(performance.status === "estimated" ? ["Google PageSpeed Lighthouse lab data"] : []), ...(profileStatus !== "measured" ? ["Google Business Profile public data"] : []), ...((local.rankingsStatus ?? local.status) !== "measured" ? ["Google organic and Maps rankings"] : [])] },
...
export function buildSiteIssues(audit: TechnicalSeoSiteAudit): TechnicalSeoIssue[] {
  ...
  const performanceQualifier = audit.performance.status === "estimated" ? "estimated" : "measured";
  if (audit.performance.mobile?.score !== null && audit.performance.mobile?.score !== undefined && audit.performance.mobile.score < 50) add("mobile-performance-poor", `Poor ${performanceQualifier} mobile performance`, "high", "Page speed", `The ${performanceQualifier} mobile lab performance score is ${audit.performance.mobile.score}/100.`, `LCP ${Math.round(audit.performance.mobile.lcpMs ?? 0)} ms; TBT ${Math.round(audit.performance.mobile.tbtMs ?? 0)} ms; CLS ${audit.performance.mobile.cls ?? "unavailable"}. ${audit.performance.reason ?? "Google PageSpeed measurement."}`, "Slow mobile delivery creates a weaker user experience and can undermine search performance in competitive results.");
  if (audit.performance.mobile?.lcpMs && audit.performance.mobile.lcpMs > 2500) add("mobile-lcp", "Slow mobile Largest Contentful Paint", audit.performance.mobile.lcpMs > 4000 ? "high" : "medium", "Core Web Vitals", `${performanceQualifier === "estimated" ? "Estimated" : "Measured"} mobile LCP is ${Math.round(audit.performance.mobile.lcpMs)} ms.`, audit.performance.reason ?? "Google PageSpeed lab measurement.", "The main visible content takes longer than the recommended threshold to appear.");
