...
export async function buildSiteAudit(args: { rootUrl: string; homepage: TechnicalSeoPageAudit; sitemapUrls: string[]; context: TechnicalSeoAuditContext; issues: TechnicalSeoIssue[]; signal?: AbortSignal }): Promise<TechnicalSeoSiteAudit> {
  ...
  const grades: TechnicalSeoSiteAudit["grades"] = [
    ...
    { key: "performance", label: "Page speed", grade: perfScore === null ? "Not assessed" : grade(perfScore), score: perfScore, rationale: performance.status === "measured" ? (performance.reason ?? "Based on the measured mobile Lighthouse performance score.") : performance.reason ?? "PageSpeed was not available." },
...
export function buildSiteIssues(audit: TechnicalSeoSiteAudit): TechnicalSeoIssue[] {
  ...
  if (audit.performance.mobile?.score !== null && audit.performance.mobile?.score !== undefined && audit.performance.mobile.score < 50) add("mobile-performance-poor", "Poor measured mobile performance", "high", "Page speed", `The mobile lab performance score is ${audit.performance.mobile.score}/100.`, `LCP ${Math.round(audit.performance.mobile.lcpMs ?? 0)} ms; TBT ${Math.round(audit.performance.mobile.tbtMs ?? 0)} ms; CLS ${audit.performance.mobile.cls ?? "unavailable"}. ${audit.performance.reason ?? "Google PageSpeed measurement."}`, "Slow mobile delivery creates a weaker user experience and can undermine search performance in competitive results.");
  if (audit.performance.mobile?.lcpMs && audit.performance.mobile.lcpMs > 2500) add("mobile-lcp", "Slow mobile Largest Contentful Paint", audit.performance.mobile.lcpMs > 4000 ? "high" : "medium", "Core Web Vitals", `Measured mobile LCP is ${Math.round(audit.performance.mobile.lcpMs)} ms.`, audit.performance.reason ?? "Google PageSpeed lab measurement.", "The main visible content takes longer than the recommended threshold to appear.");
