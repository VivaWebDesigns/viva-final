...
const INTEGRATIONS = [
  ...
  },
  {
    ...
    settings: {
      name: "OpenAI",
      description: "Vision extraction for Local Visibility Snapshot smart paste",
      docsUrl: "https://platform.openai.com/docs",
      requiredEnvVars: ["OPENAI_API_KEY"],
      optionalEnvVars: ["OPENAI_VISIBILITY_MODEL"],
      icon: "Brain",
      featureFlag: "active",
      setupInstructions: [
      ...
      operationalNotes: "OpenAI vision improves field extraction for Local Visibility Snapshot smart paste. If the key is absent or the API call fails, the tool automatically uses server-side Tesseract OCR instead.",
      usedBy: ["Local Visibility Snapshot Smart Paste", "AI Lead Scoring (planned)", "Content Generation (planned)", "Activity Summaries (planned)"],
    },
  },
  {
    ...
    settings: {
      ...
      setupInstructions: [
        ...
        "Create a separate public reports bucket and set REPORTS_R2_BUCKET_NAME",
        "Connect the reports bucket to a custom domain and set REPORTS_PUBLIC_URL",
      ],
      operationalNotes: "Cloudflare R2 stores private CRM assets. Published scan-report PNGs are copied to a dedicated public bucket and served directly from its custom domain, independently of Railway.",
