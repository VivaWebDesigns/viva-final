...
router.delete("/users/:id", requireRole("admin"), async (req, res) => {
  ...
  const [
    ...
    db.select({ count: sql<number>`count(*)::int` }).from(crmLeadNotes).where(eq(crmLeadNotes.userId, targetId)),
    db.select({ count: sql<number>`count(*)::int` }).from(clientNotes).where(eq(clientNotes.userId, targetId)),
