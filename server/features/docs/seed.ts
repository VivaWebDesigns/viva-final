...
`,
  ...
  {
    title: "Automation Tasks in Profiles & Task Views",
    slug: "automation-tasks-in-profiles",
    categorySlug: "automations",
    status: "published",
    content: `# Automation Tasks in Profiles & Task Views

Tasks generated by stage-based automations are standard follow-up tasks. They appear in all the same places as manually created tasks and support the same workflows (complete, reschedule, etc.).

## Where Automation Tasks Appear

### Profile Tasks Tab
The Tasks tab in lead/client profiles shows all tasks linked to that account. Automation-generated tasks appear alongside manual tasks and are marked with a purple **Auto** badge. Hovering over the badge shows which pipeline stage triggered the task.

### Tasks Due Today Page
The Tasks Due Today page also displays automation-generated tasks with:
- A purple **Auto** badge
- A label indicating the trigger stage (e.g., "Generated at stage: demo scheduled")

### Opportunity & Lead Detail Pages
Tasks linked to specific opportunities or leads also display the automation provenance indicators.

## Provenance Indicators

Automation-generated tasks include visual cues so users know the task origin:

| Indicator | Location | Description |
|-----------|----------|-------------|
| **Auto** badge (purple) | Task rows in all views | Shows the task was auto-generated |
| Stage label | Tasks Due Today page | "Generated at stage: {stage name}" |
| Tooltip | Profile Tasks tab | Hover over the Auto badge to see the trigger stage |

## Visibility Rules

### Admins
Admins can see all automation-generated tasks across all profiles and the Tasks Due Today page.

### Sales Reps
Sales reps see automation-generated tasks for opportunities assigned to them, following the same ownership rules as all other tasks.

## Task Workflows

Automation-generated tasks behave identically to manually created tasks:
- **Completion**: Can be marked as complete with outcomes
- **Rescheduling**: Can be rescheduled to a new date/time
- **Sorting**: Sorted by due date alongside other tasks
- **Filtering**: Appear in the same task lists and respond to the same filters
- **Status changes**: Support the same completion and outcome flows

## How It Works Behind the Scenes

When a task is auto-generated, the system creates an entry in the execution log linking the task to its source automation template. When tasks are loaded for display, the system joins against the execution log to determine which tasks were automation-generated and includes the trigger stage metadata.
