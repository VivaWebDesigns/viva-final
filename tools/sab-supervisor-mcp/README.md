...
The watcher requires the exact extension/task or permission-window structure and semantic approval controls. For site prompts it validates the action descriptor and public hostname. For tool prompts it parses the exposed JSON, requires only recognized routine browser actions, and associates tab-only read/interaction prompts with the public page in the same Chrome window. It scans every Chrome window and matching side-panel task in each poll, presses only with `AXPress`, and prefers persistent site approval when available. It also scans Claude Desktop for the exact notice `Claude reached its tool-use limit for this turn.` and activates only the exact adjacent `Continue` control, even when Claude nests that pair inside a completed response containing unrelated controls. When Claude exposes the visible notice or control without usable button semantics, the watcher uses already-authorized Screen Recording for exact on-screen text verification and Accessibility to activate the unique matched control. Resume confirmation accepts Claude's active responding/working state or disappearance of the matched control, so a historical Continue card retained in the transcript does not trigger a false retry. Altered notices, multiple Continue controls, or nearby `Cancel`/`Deny` actions fail closed.

Claude Desktop's native MCP permission cards are approved persistently only for the exact server `viva-sab-local-supervisor` and the exact tool titles `Register private SAB SOP`, `Review SAB checkpoint`, and `Review and authorize SAB scan plan`. The detector requires the semantic `Deny`, `Always allow`, and `Allow once` controls and presses only `Always allow`; every other server or tool title fails closed. The packaged LaunchAgent test approved a real tab-create, navigation, persistent-site, title-read, and page-read flow on a previously unapproved public domain and confirmed that Claude resumed.
